...
void scheduler (void){
  ...
  for(;;){
    ...
    int highest_priority = 1000; // Use a high number to ensure any priority is lower.

    // Loop over process table to find highest priority runnable process.
    acquire (&ptable.lock);
...
// Returns 0 on success and fills *info. -1 if pid not found.
int
